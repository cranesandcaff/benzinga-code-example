...
gulp.task('styles', ['clean-css'], function () {
  return gulp.src('./src/app/styles/app.scss')
    .pipe(g.rubySass())
 ...
 * Dist
 */
gulp.task('dist', ['vendors', 'assets', 'styles-dist', 'scripts-dist', 'document'], function () {
  return gulp.src('./src/app/index.html')
    .pipe(g.inject(gulp.src('./public/vendors.min.{js,css}'), {ignorePath: 'public', starttag: '<!-- inject:vendor:{{ext}} -->'}))
    .pipe(g.inject(gulp.src('./public/' + bower.name + '.min.{js,css}'), {ignorePath: 'public'}))
    .pipe(g.htmlmin(htmlminOpts))
    .pipe(g.rename('index.ejs'))
...
function buildTemplates () {
  return lazypipe()
    .pipe(g.ngHtml2js, {
      moduleName: 'benzingaStockExchangeApp',
      stripPrefix: '/src/app'
    })
...
function dist (ext, name, opt) {
  ...
  return lazypipe()
    ...
    .pipe(opt.ngAnnotate ? gulp.dest : noop, './public')
...
/**
 * I didn't write the above code, came with my slush generator.
 */
 ...
 gulp.task('document', function () {
   ...
   return appFiles()
           .pipe(g.jsdoc.parser())
          .pipe(g.jsdoc.generator('./public/documentation', templateOptions));
 });
